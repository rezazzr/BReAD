...
def get_result(
    ...
    if nonO_correct_counts > 0:
        ...
        print(
            "accuracy: %6.2f%%; " % (100 * sum_correct_counts / sum_true_counts), end=""
        )
    ...
    for t in chunk_types:
        ...
        print("%17s: " % t, end="")
        print(
            "precision: %6.2f%%; recall: %6.2f%%; FB1: %6.2f%%" % (prec, rec, f1),
            end="",
        )
...
class CustomDataLoader:
    ...
    def _collate_fn(self, batch_data):
        ...
        question, answers = zip(
            *[(item["question"], item["answer"]) for item in batch_data]
        )  # Changed to tags
        return {
            "question": question,
            "answer": answers,
...
class CustomTask(BaseTask):
    ...
    def load_task_dataset(self, data_dir):
        return load_dataset("ncbi_disease").filter(
            lambda example: len(example["ner_tags"]) > 0
        )
    ...
    def convert_to_BIO(number_tag):
        ...
        converted_tag = [
            str(x).replace("0", "O").replace("1", "B").replace("2", "I")
            for x in number_tag
        ]
    ...
    def transform_format(self, data_dict):
        ...
        for split_name, data_split in data_dict.items():
            ...
            if "tokens" in available_columns and "ner_tags" in available_columns:
                for tokens, ner_tags in zip(
                    data_split["tokens"], data_split["ner_tags"]
                ):
                    question = tokens
                    answer = self.extract_entities(tokens, ner_tags)
                    formatted_example = {"question": question, "answer": answer}
                    formatted_data.append(formatted_example)
                transformed_data_dict[split_name] = formatted_data
            else:
                print(
                    f"Columns 'tokens' and/or 'ner_tags' not found in {split_name} data_split."
                )
    ...
    def clean_labels(self, answers):
        ...
        for answer in answers:
            ...
            # Check if the current answer is a list
                ...
                cleaned_list = [
                    item.lower() for item in answer if isinstance(item, str)
                ]
    ...
    def create_bio_preds(self, questions, predictions):
        ...
        for q, p in zip(questions, predictions):
            ...
            bio_pred_seq = " ".join(
                ["O" if (w != "B" and w != "I") else w for w in bio_pred_seq.split()]
            )
    ...
    def clean_response(response):
        ...
        for match in matches:
            ...
            entities.extend(
                [e.strip().lower() for e in normalized_match.split(",") if e.strip()]
            )
