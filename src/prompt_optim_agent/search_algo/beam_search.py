...
from .base_algo import Action, SearchAlgo, State

# TODO: Add wandb logging to this module, the same way it is implemented in MCTS
...
class BeamSearch(SearchAlgo):
    ...
    def expand(self, node: BeamNode):
        ...
        while True:
            ...
            if (
                gradient_descent_output["acc"] == -1
            ):  # TODO: check if this is correct, I don't think we have a trigger to set this to -1 but rather it should be 1 or we need to change the code elsewher in the word_model
    ...
    def search(self, init_state: str, **kwargs):
        ...
        nodes = sorted(
            nodes, key=lambda node: self._sort_helper(node.eval_metric), reverse=True
        )[: self.beam_width]
        self.nodes = nodes
        ...
        for i in range(self.depth_limit):
            ...
            nodes = sorted(
                nodes,
                key=lambda node: self._sort_helper(node.eval_metric),
                reverse=True,
            )[: self.beam_width]
    ...
    def prepare_output(self):
        ...
        best_path = sorted(
            paths_nodes,
            key=lambda path: self._sort_helper(path[-1].eval_metric),
            reverse=True,
        )[0]
        best_node = sorted(
            self.all_nodes,
    ...
    def output_to_json(self, output):
        data_to_save = {}
        paths = [[node.to_dict() for node in path] for path in output["all_paths"]]
