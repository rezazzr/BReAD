...
class CustomTask(BaseTask):
    ...
    def load_task_dataset(self, **kwargs):
        ...
        for example in dataset["train"]:
            ...
            new_dataset["train"].append(
                dict(question=question_str, answer=chr(65 + example["coarse_label"]))
            )
        for example in dataset["test"]:
            question_str = question_format.format(
                text=example["text"],
            )
            new_dataset["test"].append(
                dict(question=question_str, answer=chr(65 + example["coarse_label"]))
            )
    ...
    def clean_response(self, response):
        letters = (
            string.ascii_uppercase[: len(self.options)]
            + string.ascii_lowercase[: len(self.options)]
        )
